...
(function(){
  ...
  function truncateKaTeXSafe(input, limit = 100, katexDelimiters) {
    ...
    while(i<s.length && out.length<limit){ let next=s.length; for(const {left} of delimiters){ const idx=s.indexOf(left,i); if(idx>=0 && idx<next) next=idx; }
  ...
  function renderBookmarks(bookmarksByTag, data, opts){
    ...
    for (const tagName of sortedTags){ const bookmarks = bookmarksByTag[tagName]; const tagId = bookmarks && bookmarks.length ? bookmarks[0].tagId : null; html += `
      ...
      for (const b of bookmarks){
        if (b.kind === 'pyq') {
          ...
          const q = pyqsData.get(key)?.questions?.[b.questionIndex];
          const text = q?.qText || 'Preview loading...';
          const truncated = truncateKaTeXSafe(text, TRUNCATE_LIMIT, katexOptions?.delimiters);
          const safe = sanitizeHtml(truncated);
          html += `
          <div class="col">
            <div class="as-card card h-100 bookmark-card" data-kind="pyq" data-exam-id="${escapeHtml(String(b.examId))}" data-subject-id="${escapeHtml(String(b.subjectId))}" data-chapter-id="${escapeHtml(String(b.chapterId))}" data-question-index="${escapeHtml(String(b.questionIndex))}" data-preview-ready="${q ? 'true' : 'false'}" style="cursor:pointer;">
              <div class="card-body">
                <div class="d-flex justify-content-between align-items-start mb-2">
                  <small class="text-muted bookmark-question-label">PYQs · Q${Number(b.questionIndex) + 1}</small>
                ...
                <p class="card-text bookmark-preview">${safe}</p>
                <div class="d-flex justify-content-between align-items-center">
                  <span class="badge bg-info bookmark-qtype">${escapeHtml(String(q?.qType || 'PYQ'))}</span>
          ...
          const assignment = assignmentData.get(assignmentId);
          const q = assignment?.questions?.[b.questionIndex];
          const displayIndex = assignment ? assignment.questions.slice(0,b.questionIndex+1).filter(qq=>qq.qType!=='Passage').length - 1 : Number(b.questionIndex);
          const text = q?.qText || 'Preview loading...';
          const truncated = truncateKaTeXSafe(text, TRUNCATE_LIMIT, katexOptions?.delimiters);
          const title = assignmentTitles.get(assignmentId) || `Assignment ${assignmentId}`;
          html += `
          <div class="col">
            <div class="as-card card h-100 bookmark-card" data-kind="assignment" data-assignment-id="${escapeHtml(String(assignmentId))}" data-question-index="${escapeHtml(String(b.questionIndex))}" data-display-index="${escapeHtml(String(displayIndex))}" data-preview-ready="${q ? 'true' : 'false'}" style="cursor:pointer;">
              <div class="card-body">
                <div class="d-flex justify-content-between align-items-start mb-2">
                  <small class="text-muted bookmark-question-label">${escapeHtml(title)} · Q${displayIndex + 1}</small>
                  <button class="btn btn-sm btn-outline-danger remove-bookmark" data-kind="assignment" data-assignment-id="${escapeHtml(String(assignmentId))}" data-question-index="${escapeHtml(String(b.questionIndex))}" data-tag-id="${escapeHtml(String(b.tagId))}"><i class="bi bi-x"></i></button>
                </div>
                <p class="card-text bookmark-preview">${escapeHtml(truncated).replace(/\n/g,'<br>')}</p>
                <div class="d-flex justify-content-between align-items-center">
                  <span class="badge bg-info bookmark-qtype">${escapeHtml(String(q?.qType || 'Assignment'))}</span>
                  <small class="text-muted">${formatDate(b.created_at)}</small>
                </div>
              </div>
            </div>
    ...
    document.querySelectorAll('.bookmark-card').forEach((card) => {
      card.addEventListener('click', (e) => {
        ...
        if (kind === 'pyq') onShow?.({ kind: 'pyq', examId: card.dataset.examId, subjectId: card.dataset.subjectId, chapterId: card.dataset.chapterId, questionIndex: Number(card.dataset.questionIndex) });
        else onShow?.({ kind: 'assignment', assignmentId: Number(card.dataset.assignmentId), questionIndex: Number(card.dataset.questionIndex) });
      });
    });
    ...
    document.querySelectorAll('.remove-bookmark').forEach((btn) => {
      btn.addEventListener('click', async (e) => {
        ...
        let ok = false;
        if (kind === 'pyq') ok = await onRemove?.({ kind: 'pyq', examId: btn.dataset.examId, subjectId: btn.dataset.subjectId, chapterId: btn.dataset.chapterId, questionIndex: Number(btn.dataset.questionIndex), tagId: btn.dataset.tagId });
        else ok = await onRemove?.({ kind: 'assignment', assignmentId: Number(btn.dataset.assignmentId), questionIndex: Number(btn.dataset.questionIndex), tagId: btn.dataset.tagId });
  ...
  function updateBookmarkCardPreview(card, preview, katexOptions){
    if (!card || !preview) return;
    const labelEl = card.querySelector('.bookmark-question-label');
    const previewEl = card.querySelector('.bookmark-preview');
    const qTypeEl = card.querySelector('.bookmark-qtype');
    if (labelEl && preview.labelText) labelEl.textContent = preview.labelText;
    if (previewEl) {
      if (preview.isHtml) previewEl.innerHTML = sanitizeHtml(truncateKaTeXSafe(preview.text || '', 100, katexOptions?.delimiters));
      else previewEl.innerHTML = escapeHtml(truncateKaTeXSafe(preview.text || '', 100, katexOptions?.delimiters)).replace(/\n/g,'<br>');
      try { if (window.renderMathInElement) window.renderMathInElement(previewEl, katexOptions); } catch {}
    }
    if (qTypeEl && preview.qType) qTypeEl.textContent = String(preview.qType);
    if (preview.displayIndex !== undefined) card.dataset.displayIndex = String(preview.displayIndex);
  ...
  async function showQuestion(assignmentId, questionIndex, assignmentData, assignmentTitles){
    ...
    openBtn.dataset.assignmentId = assignmentId; openBtn.dataset.questionIndex = String(qIdx); openBtn.dataset.displayIndex = String(displayIndex); delete openBtn.dataset.examId; delete openBtn.dataset.subjectId; delete openBtn.dataset.chapterId; delete openBtn.dataset.qIndex; try { openBtn.textContent = 'Open in Assignment'; } catch {}
  ...
  function normalizeAnswer(q){ if (q.qAnswer && typeof q.qAnswer==='object' && !Array.isArray(q.qAnswer)) { const kind=String(q.qAnswer.kind||'').toLowerCase(); const alternatives=Array.isArray(q.qAnswer.alternatives)? q.qAnswer.alternatives:[]; if (kind==='numerical'){ const first=alternatives[0]; if (first?.mode==='range') return { value:`${first.start} to ${first.end}`, valid:Number.isFinite(Number(first.start)) && Number.isFinite(Number(first.end)) }; const n=Number(first?.value); return { value:n, valid:!Number.isNaN(n) }; } return new Set(alternatives.flatMap((entry)=> (Array.isArray(entry)? entry:[entry])).map((x)=>String(x).trim().toUpperCase()).filter(Boolean)); } if (q.qType==='SMCQ') return new Set([String(q.qAnswer).trim().toUpperCase()]); if (q.qType==='MMCQ'){ const arr = Array.isArray(q.qAnswer)? q.qAnswer : [q.qAnswer]; return new Set(arr.map(x=>String(x).trim().toUpperCase())); } if (q.qType==='Numerical'){ const n=Number(q.qAnswer); return { value:n, valid: !Number.isNaN(n) }; } return new Set(); }
  ...
  async function showPyqQuestion(examId, subjectId, chapterId, questionIndex, pyqsData){
    ...
    titleEl.textContent = `PYQs · Question ${qIdx + 1}`; delete openBtn.dataset.assignmentId; delete openBtn.dataset.questionIndex; delete openBtn.dataset.displayIndex; openBtn.dataset.examId = String(examId); openBtn.dataset.subjectId = String(subjectId); openBtn.dataset.chapterId = String(chapterId); openBtn.dataset.qIndex = String(qIdx); try { openBtn.textContent = 'Open in PYQs'; } catch {}
    const esc = (v) => String(v ?? '').replace(/&/g,'&amp;').replace(/</g,'&lt;').replace(/>/g,'&gt;').replace(/\"/g,'&quot;').replace(/'/g,'&#39;');
