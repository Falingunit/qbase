...
function setLoggedOutUI() {
  ...
  function ensureLoginGate() {
    ...
    wrap.className = 'position-fixed top-0 start-0 w-100 h-100 align-items-center justify-content-center';
    wrap.style.cssText = 'z-index:2000;background:rgba(0,0,0,0.85);backdrop-filter:saturate(120%) blur(2px)';
    wrap.innerHTML = `
    ...
    const onSubmit = (e) => { e?.preventDefault?.(); gateLoginFlow(); };
    btn?.addEventListener('click', onSubmit);
    input?.addEventListener('keydown', (e) => { if (e.key === 'Enter') onSubmit(e); });
  ...
  function broadcastLogin(name) {
    window.dispatchEvent(new CustomEvent('qbase:login', { detail: { username: name } }));
  }
  ...
  function showModal({ title, bodyHTML, buttons, focusSelector, backdrop = 'static', keyboard = true, onContentReady }) {
  ...
  // 4) Public helpers
  async function showConfirm({ title, message, okText='OK', cancelText='Cancel' }) {
    return await showModal({
      title,
      bodyHTML: `<p class="mb-0">${message}</p>`,
      buttons: [
        { text: cancelText, className: 'btn btn-outline-secondary', value: false },
        { text: okText,     className: 'btn btn-success',           value: true }
      ]
    });
  }

  async function showPrompt({ title, label='Enter value', placeholder='', okText='OK', cancelText='Cancel', initial='' }) {
    const id = 'qbasePromptInput';
    const body = `
      <label for="${id}" class="form-label">${label}</label>
      <input id="${id}" class="form-control" type="text" placeholder="${placeholder}" value="${initial ?? ''}">
    `;
    const res = await showModal({
      title,
      bodyHTML: body,
      buttons: [
        { text: cancelText, className: 'btn btn-outline-secondary', value: { ok:false, value:null } },
        { text: okText,     className: 'btn btn-primary',           value: { ok:true,  value:null } }
      ],
      focusSelector: `#${id}`
    });
  ...
  function listLocalAssignmentStates() { return []; }
  ...
  async function migrateAllLocalAssignmentsIfAny() { /* no-op: local storage disabled */ }

  // -------------------- Login / Logout flows --------------------
  async function doLoginFlow() { showLoginGate(); }
  ...
  async function gateLoginFlow() {
    ...
    if (!username || username.length < 2) {
      if (err) { err.textContent = 'Please enter a valid username (min 2 characters).'; err.style.display = 'block'; }
      input?.focus();
      return;
    }
  ...
  async function doDeleteAccountFlow() {
    const ok = await showConfirm({
      ...
      message: 'This will permanently delete your account and ALL saved progress. This action cannot be undone. Do you want to continue?',
      okText: 'Delete',
      cancelText: 'Cancel'
    });
    if (!ok) return;
    ...
    try {
      ...
      await showNotice({ title: 'Error', message: 'Failed to delete account. Please try again.' });
    }
  }

  // -------------------- Wire up & initialize --------------------
  loginLink?.addEventListener('click', (e) => { e.preventDefault(); showLoginGate(); });
  logoutLink?.addEventListener('click', (e) => { e.preventDefault(); doLogoutFlow(); });
  deleteAccountLink?.addEventListener('click', (e) => { e.preventDefault(); doDeleteAccountFlow(); });
