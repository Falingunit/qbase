  ...
  buildToolbar(
    ...
    ({ q }) => {
      ...
      if (els.starredExamsWrap && els.starredExamsGrid) {
        ...
        const starredList = exams.filter(
          (e) => starredExams.has(e.id) && matchExam(e, q)
        );
        if (starredList.length > 0) {
          starredList.forEach((it) =>
            els.starredExamsGrid.appendChild(examCard(it, { onToggleStar }))
          );
          els.starredExamsWrap.classList.remove("d-none");
          if (els.starredExamsCount)
            els.starredExamsCount.textContent = `(${starredList.length})`;
        } else {
...
function matchExam(exam, q) {
  const hay = `${exam.name}`.toLowerCase();
...
function examCard(exam, { onToggleStar }) {
  ...
  img.loading = "lazy";
  img.decoding = "async";
  img.referrerPolicy = 'no-referrer';              // NEW
  img.crossOrigin = 'anonymous'; 
  img.src = exam.icon || ICON_FALLBACK;
  img.onerror = () => {
    img.onerror = null;
